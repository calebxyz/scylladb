...
BOOST_AUTO_TEST_CASE(metadata_id_changed_rows_response_overrides_no_metadata) {
    ...
    cql_transport::response response(0, cql_transport::cql_binary_opcode::RESULT, {});
    response.write(rows_metadata, cql_transport::cql_metadata_id_wrapper(
            std::move(empty_metadata_id),
            std::move(stale_response_metadata_id)), true);

    auto body_stream = std::move(response).extract_body();
