...
// we have to different ways to handle this, if we are using the old gossip algorithm for topologu we will use the old 
// way of doing things, where updates will be done by switch_tenant function that will ensure async execution 
// because gossip algorithm is temporary its easier to make the changes just for the new algorithm and then remove the 
// old way of doing things. if we are on v2 server then this function will be not asynchronius and be updated fro ma cache 
void cql_server::connection::update_scheduling_group() {
    if (_server._sl_controller.is_v2()) {
        update_user_scheduling_group_sync();
    } else { 
        update_user_scheduling_group_async();
    }
}


void cql_server::connection::update_user_scheduling_group_sync() {
...
void cql_server::connection::update_user_scheduling_group_async() {
    switch_tenant([this] (noncopyable_function<future<> ()> process_loop) -> future<> {
